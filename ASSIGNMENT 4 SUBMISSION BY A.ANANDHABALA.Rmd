---
title: "ASSIGNMENT 4 SUBMISSION BY A,ANANDHABALA"
...
library(MASS)
library(ISLR)
### for the new package for bootstrap using to find re sampling in observation minimum standard error using ### BOOTSTRAP ### 
library(boot)
```
...
ANS:
```{r}
### using set.seed is its cant give the repeated observation we use set.seed. ###
set.seed(1)
### add to the default data set. ###
attach(Default)
```

```{r}
### Logarthmic regression formula for income and balance in default using glm.###
glm.fit = glm(default~income+balance, data=Default, family="binomial")
### get the summary information using summary command,###
summary(glm.fit)
...
ANS:
```{r}
### Bootstrap using for re sample use same default data set using index is mention number of re sampling.
boot.fn = function(data, index){
### logaristic regression for linear model between income and balance
...
```{r}
### 1000 is number time re sampling is 1000.
boot(Default, boot.fn, 100)
```
Ans:
    using boot  function to default data set 1000 times re sampling.

 (d) Comment on the estimated standard errors obtained using the glm() function and using your bootstrap function.
Ans:

```{r}
### glm function using bootstrap.###
###glm.fit = glm(default~income+balance, data=Default, family="binomial")
###summary(glm.fit)
boot.fn=function(data,index){
  return(glm(default~income+balance, data=Default, family="binomial",subset=index))
}
boot.fn(default,1:nrow(Default))
```
...
ANS:
```{r}
### not repeated observation using set seed of weekly dataset ###
set.seed(1)
Weekly
...
```{r}
### logaristic regression using linear model in between  Lag1 + Lag2 in weekly dataset.
...
```{r}
### its predict the regression model of  Lag1 , Lag2using glm.
...
```{r}
### its predict the weekly exclude first riw  its greater than 0.5 that was predict in glm.fit2
predict(glm.fit2, Weekly[1, ], type="response")>0.5

```

```{r}
### exclude first observation  .get the direction from the weekly data set.
Weekly[1,]$Direction
```
...
```{r}
### using loop for pred is greater than 0.5 that is set in the "up".others set for the "down" 
### up value set in 1 down value set in 0 using glm and loop function .
...
```{r}
### get the average of error rate is 0.4499541 using mean.
mean(error)
```


                ### ASSIGNMENT 4 SUBMISSION BY A,ANANDHABALA. ### 